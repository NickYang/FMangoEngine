...
static void makepath(std::string path, int mode)
{
    ...
    for (unsigned int i = 0; i < dirs.size(); i++)
    {
        ...
        if (stat(dirPath.c_str(), &s) != 0)
        {
            ...
            {
                FM_ERROR("Failed to create directory: '%s'", dirPath.c_str());
                return;
            }
        }
    }
...
char* FileSystem::readAll(const char* filePath, int* fileSize)
{
    ...
    if (stream.get() == NULL)
    {
        FM_ERROR("Failed to load file: %s", filePath);
        return NULL;
    }
    ...
    if (read != size)
    {
        FM_ERROR("Failed to read complete contents of file '%s' (amount read vs. file size: %u < %u).", filePath, read, size);
    ...
    if (upToDateAssets.find(fullPath) == upToDateAssets.end())
    {
        ...
        if (asset)
        {
            ...
            if (file != NULL)
            {
                ...
                if (fclose(file) != 0)
                {
                    FM_ERROR("Failed to close file on file system created from APK asset '%s'.", path);
                    return;
                }
                if (ret != length)
                {
                    FM_ERROR("Failed to write all data from APK asset '%s' to file on file system.", path);
                    return;
                }
            }
            else
            {
                FM_ERROR("Failed to create file on file system from APK asset '%s'.", path);
                return;
            }
